...
clonal_peaks = readRDS('input_data_P05/peak_per_pathways_filtered_tss1kb_peaks.rds')
...
pathways_epigenetic_classes = readRDS('input_data_P05/a_scores_scaled_per_group_filtered_tss1kb_peaks.rds')
...
p05_peaks <- p05_peaks %>% 
  select(peak,pathway) %>% 
  distinct()
saveRDS(object = p05_peaks,file = 'input_data_P05/all_peaks_filtered_085_tss1kb_df.rds')
